...
from arbiscan.lifecycle import ActionabilityPolicy, LifecycleReason, LifecycleState, revalidate_opportunity

NOW = datetime(2026, 9, 15, 18, 0, tzinfo=UTC)
EUR = CurrencyRoundingPolicy(currency="EUR", quantum=Decimal("0.01"))


def quote(selection: str, odds: str, index: int, *, age: int = 0, status: QuoteStatus = QuoteStatus.ACTIVE) -> OddsQuote:
    return OddsQuote(
        id=QuoteId(f"quote:{index}"), provider_id=ProviderId(f"provider:{index}"),
        event_id=EventId("event:phase12"), market_id=MarketId("market:phase12"),
        selection_id=SelectionId(selection), decimal_price=Decimal(odds),
        source_event_id="source:event", source_market_id="source:market",
        source_selection_id=selection, ingested_at=NOW - timedelta(seconds=age),
        status=status, trace_id=f"trace:{index}",
    )


def opportunity(quotes: tuple[OddsQuote, ...]):
    evaluation = evaluate_market(quotes, tuple(item.selection_id for item in quotes))
    return build_opportunity(evaluation, opportunity_id=OpportunityId("opportunity:phase12"), detected_at=NOW)


def policy(**overrides: object) -> ActionabilityPolicy:
    values: dict[str, object] = {
        "bankroll": Decimal("100"), "rounding_policy": EUR,
        "maximum_quote_age_seconds": Decimal("30"),
    }
    values.update(overrides)
    return ActionabilityPolicy(**values)  # type: ignore[arg-type]


def test_fresh_profitable_opportunity_becomes_actionable() -> None:
    quotes = (quote("a", "2.20", 1), quote("b", "2.20", 2))
    result = revalidate_opportunity(opportunity(quotes), quotes, now=NOW, stake_plan_id=StakePlanId("plan:1"), policy=policy())
...
def test_stale_quote_fails_closed_before_staking() -> None:
    ...
    result = revalidate_opportunity(opportunity(detected), current, now=NOW, stake_plan_id=StakePlanId("plan:stale"), policy=policy())
...
def test_closed_market_expires_opportunity() -> None:
    detected = (quote("a", "2.20", 1), quote("b", "2.20", 2))
    current = (quote("a", "2.20", 1, status=QuoteStatus.SUSPENDED), quote("b", "2.20", 2))
    result = revalidate_opportunity(opportunity(detected), current, now=NOW, stake_plan_id=StakePlanId("plan:closed"), policy=policy())
...
def test_price_drift_that_removes_arbitrage_invalidates() -> None:
    ...
    result = revalidate_opportunity(opportunity(detected), current, now=NOW, stake_plan_id=StakePlanId("plan:drift"), policy=policy())
...
def test_post_rounding_net_profit_threshold_is_enforced() -> None:
    ...
    result = revalidate_opportunity(
        opportunity(quotes), quotes, now=NOW, stake_plan_id=StakePlanId("plan:fees"),
        policy=policy(commission_rate=Decimal("0.20"), tax_rate=Decimal("0.25"), minimum_guaranteed_profit=Decimal("7")),
    )
...
def test_maximum_exposure_and_minimum_roi_are_applied() -> None:
    quotes = (quote("a", "2.20", 1), quote("b", "2.20", 2))
    result = revalidate_opportunity(
        opportunity(quotes), quotes, now=NOW, stake_plan_id=StakePlanId("plan:exposure"),
